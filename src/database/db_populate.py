...
def insert_batch(cursor,logger,meta,price,power):
    ...
    for ft in power.keys():
        for ts, pw in power[ft]['data']:
            try:
                ...
                print(f"failed to insert ('{batch_index}', '{ts}', '{ft_indexes[ft]}', '{pw}') into Fact_Power")

    ## FACT_PRICE
    logger.debug('FACT_PRICE')
    for ts, pc in price['data']:
        try:
            ...
            print(f"failed to insert ('{batch_index}', '{ts}', '{pc}') into Fact_Price")

    ## FACT_X_CAP
    logger.debug('FACT_X_CAP')
    for ft in power.keys():
        try:
            cursor.execute(
            ...
            print(f"failed to insert ('{batch_index}', '{ft_indexes[ft]}', '{power[ft]['meta']['x_capacity_at_present']}') into Fact_X_Cap")
